...
class Chip8 {
 public:
  // variables
  uint8_t registers8_8[16]{};         // 8-bit (1 Byte) registers - 16
 ...
 private:
  ...
  void Op_8xy6();  // 16) Vy >> Vx; Shift Right Vy (1-bit) and store in Vx; Don't
                   // modify Vy; Store Vy's LSB in Vf before shifting
  ...
  void Op_Ex9E();  // 24) Skip next instruction if key with value in Vx is pressed
  void Op_ExA1();  // 25) Skip next instruction if key with value in Vx is ~pressed
  ...
  void Op_Fx29();  // 31) Set index16 = address(sprite => corresp. 0x digit in Vx)
  ...
  std::mt19937 RandomGenerator;                        // Random engine
  std::uniform_int_distribution<uint8_t> random_byte;  // Will be used for
                                                       // range of random nums
                                                       // to be generated
};
